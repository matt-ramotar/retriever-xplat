...
class RealChannelRepository(
    ...
    override fun stream(userId: String): Flow<List<Channel.Output.Unpopulated>?> = channelFlow {
        channelsStore.stream<Channel.Output.Unpopulated>(StoreReadRequest.fresh(userId)).map { storeReadResponse ->
            send(storeReadResponse.dataOrNull())
        }
    }

    override suspend fun get(channelId: String): Channel.Output.Populated? =
        channelStore.stream<Channel.Output.Populated>(StoreReadRequest.fresh(channelId)).firstOrNull()?.dataOrNull()
}